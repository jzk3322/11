...
static status_log_t status_log[16];    // array size must be power of 2 and <=256
static uint8_t status_log_read;
static uint8_t status_log_write;
    
// status log object queue
static uint32_t status_log_obj[16];    // array size must be power of 2 and <=256
static uint8_t status_log_obj_read;
static uint8_t status_log_obj_write;
...
void status_log_event(uint8_t status, uint32_t *obj)
{
   if (((status_log_write - status_log_read) & STATUS_LOG_MASK) >= STATUS_LOG_MASK) {
    ...
    case STATUS_LOG_MSG_TX:
        log->msg_header = PD_protocol_get_tx_msg_header(&app_pt.protocol);
        log->obj_count = status_log_obj_add(log->msg_header, obj);
        break;
    case STATUS_LOG_MSG_RX:
        log->msg_header = PD_protocol_get_rx_msg_header(&app_pt.protocol);
        log->obj_count = status_log_obj_add(log->msg_header, obj);
        break;
    default:
        break;
    ...
    log->status = status;
    log->time = xTaskGetTickCount()&0xffff;
    status_log_write++;
}
...
int status_log_readline_src_cap(char * buffer, int maxlen)
{
    ...
    if (PD_protocol_get_power_info(&app_pt.protocol, i, &p)) {
        const char * str_pps[] = {"", " BAT", " VAR", " PPS"};  /* PD_power_data_obj_type_t */
        char * t = status_log_time;
        uint8_t selected = PD_protocol_get_selected_power(&app_pt.protocol);
...
int status_log_readline(char *buffer, int maxlen)
{
    ...
    case STATUS_LOG_MSG_RX:
        ...
        break;
    case STATUS_LOG_DEV:
        if (app_pt.status_initialized) {
            uint8_t version_ID = 0, revision_ID = 0;
            FUSB302_get_ID(&board.fusb302, &version_ID, &revision_ID);
    ...
    case STATUS_LOG_POWER_READY: {
        uint16_t v = app_pt.ready_voltage;
        uint16_t a = app_pt.ready_current;
        if (app_pt.status_power == STATUS_POWER_TYP) {
            LOG("%s%d.%02dV %d.%02dA supply ready\n", t, v / 20, (v * 5) % 100, a / 100, a % 100);
        } else if (app_pt.status_power == STATUS_POWER_PPS) {
