...
const uint8_t FAT16_RootDirSector[FATDirSize]= 
{
    ...
    0x00,           /*30 - File Size */
    0x00,           /*31 - File Size */
    'F','L',' ', 'P','O','W','E','R',' ',' ',' ',  /*32-42 - Volume label */
...
{
    uint32_t flashEndAddr =  board.ap_address + board.ap_size - 1;;
    ...
    if (!memcmp(&(FileAttr.DIR_Name[8]), "BIN", 3))
    {
        ...
        if(board.ap_size >= FileAttr.DIR_FileSize)
        {
            for( i = 0; i < len; i+=2 )
            {
                writeAddr = board.ap_address + dataTimes * board.page_size + (i % board.page_size);
                if(( writeAddr < board.ap_address ) || (writeAddr > flashEndAddr))//check flash addr is right.
                {
                    fat_event_set(FAT_EVENT_ERRAPP);
                    break; 
                }
                if((writeAddr - board.ap_address) % board.page_size == 0)
                {
                    writeBase = board.ap_address + dataTimes * board.page_size;
                    dataTimes += 1;
                    FLASH_Unlock();
                    FLASH_ErasePage(writeBase);
                }
                FLASH_Unlock();
                FLASH_ProgramHalfWord(writeBase + (i % board.page_size), (data[i+1]<<8) + data[i]);
            }
            *filesize_write += len;
            if(*filesize_write >= filesize_total)
            {
                *filesize_write = 0;
                dataTimes = 0;
                fat_event_set(FAT_EVENT_SUCCESS);
            }
        }
        else
        {
            fat_event_set(FAT_EVENT_LARGE);
        }
    }
    else if(!memcmp(&(FileAttr.DIR_Name[8]), "HEX", 3))
    {
        ...
        for(i = 0; i < len; i++)
        {
            ...
            if(HEX_E_OK == result)
            {
                ...
                {
					if((mData.addr < board.ap_address ) || (mData.addr > flashEndAddr))//check flash addr is right.
					{
                        fat_event_set(FAT_EVENT_ERRAPP);
						break; 
					}
                    
                    if((mData.addr - board.ap_address) % board.page_size == 0)
                    {
                        FLASH_Unlock();
    ...
    else if(!memcmp(&(FileAttr.DIR_Name[8]), "SEC", 3))
    {
        ...
        // De-Encry
        // Add Your Code Here
        fat_event_set(FAT_EVENT_UNKOWN);
    }
    else
    {
        // Can't Recognize it
...
void fat_event_set(uint16_t info)
{
    fat_event.info = info;
    fat_event.change = 1;
}